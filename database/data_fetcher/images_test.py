...
async def process_anime(session, conn, anime_record, semaphore):
    async with semaphore:
        ...
        image_url = next((img['jpg']['image_url'] for img in pics_data['data']
                          if img.get('jpg') and img['jpg'].get('image_url')), None)
    ...
    try:
        ...
        anime_records = await conn.fetch("""
            ...
            WHERE NOT EXISTS (
                ...
                WHERE m.entity_type = 'anime' AND m.entity_id = a.anime_id AND m.media_type = 'image'
            )
            ORDER BY anime_id
        """)
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [process_anime(session, conn, anime, semaphore) for anime in anime_records]
